...
export function setHoverBackgroundElement() {
  $(".about-vision-mission .section-content").each(function () {
    const $section = $(this);
    const defaultBg = $section.attr("data-default-bg");

    // Background mặc định
    if (defaultBg) {
      $section.css("background-image", `url(${defaultBg})`);
    }

    $section.find(".item").on("mouseenter", function () {
      const $item = $(this);
      const bg = $item.attr("data-bg");

      // Active item hiện tại
      $section.find(".item").removeClass("is-active");
      $item.addClass("is-active");

      // Active overlay
      $section.addClass("is-hover");
    ...
    $section.find(".item").on("mouseleave", function () {
      // Chỉ reset khi chuột đã rời toàn bộ section
      if (!$section.find(".item:hover").length) {
        $section.removeClass("is-hover");
        $section.find(".item").removeClass("is-active");
